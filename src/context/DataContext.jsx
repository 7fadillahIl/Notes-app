...
const DataProvider = ({ children }) => {
  ...
  useEffect(() => {
    initialData.filter(
      (item) => (item.createdAt = showFormattedDate(item.createdAt))
    );
    setData(initialData);
  }, []);
  ...
  const inputHandler = (e) => {
    ...
    countRef.current.value.length <= maxLength &&
  ...
  return (
    <DataContext.Provider value={allData}>
      <ItemContext.Provider value={allItem}>{children}</ItemContext.Provider>
